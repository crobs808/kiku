...
impl Config {
    ...
    pub fn build(&mut self) -> PathBuf {
        ...
        // Android NDK defaults to armeabi-v7a unless ANDROID_ABI is set.
        // When cargo cross-compiles to android with an explicit NDK toolchain file,
        // infer the ABI from rust target arch unless a caller already set one.
        if self.defined("CMAKE_TOOLCHAIN_FILE")
            && getenv_unwrap("CARGO_CFG_TARGET_OS") == "android"
            && !self.defined("ANDROID_ABI")
        {
            if let Some(android_abi) = match getenv_unwrap("CARGO_CFG_TARGET_ARCH").as_str() {
                ...
                self.define("ANDROID_ABI", android_abi);
            }
        }
